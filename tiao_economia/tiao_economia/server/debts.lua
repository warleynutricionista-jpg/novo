--============================================================
-- space_economy - server/debts.lua
-- Registro de dívidas e aplicação de juros
--============================================================
SE = SE or {}
...
function SE.Debts.GetOpenDebtsFor(citizenId, cb)
  if not MySQL then
    if cb then cb({}) end
    return
  end
  MySQL.query('SELECT * FROM se_debts WHERE citizen_id = ? AND status = "open"', { citizenId }, function(res)
    if cb then cb(res) end
  end)
end

-- Aplica juros diários (executar por cron interno)
CreateThread(function()
  local interval = 60 * 60 * 1000 -- 1 hora (ver Config.DebtSystem.WarnEveryHours se quiser)
  while true do
    Wait(interval)
    if not (Config and Config.DebtSystem and Config.DebtSystem.Enabled) then goto continue end
    -- aplicar juros em todas as dívidas abertas
    if MySQL then
...
-- Pagar dívida (integração simples)
RegisterNetEvent('space_economy:server_payDebt', function(debtId)
  ...
  MySQL.query('SELECT * FROM se_debts WHERE id = ? LIMIT 1', { debtId }, function(rows)
    ...
    -- remover dinheiro do jogador (integração)
    -- reusar função de tax.lua se existir
    local success = false
    if SE.Tax and SE.Tax._removePlayerMoney then
      success = SE.Tax._removePlayerMoney(src, amount)
    else
      TriggerClientEvent('space_economy:client_notify', src, 'Integração de pagamento ausente', 'error')
      return
    end
    if success then
      ...
      TriggerClientEvent('space_economy:client_notify', src, 'Falha ao pagar dívida (fundos?)', 'error')
    end
  end)
end)
